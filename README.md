...
| <a name="input_protection_plans"></a> [protection\_plans](#input\_protection\_plans) | Provides a resource to manage a GuardDuty malware protection plan. | <pre>map(object({<br/>    role   = string<br/>    region = optional(string)<br/>    tags   = optional(map(string))<br/><br/>    actions = optional(object({<br/>      tagging = object({<br/>        status = string<br/>      })<br/>    }))<br/><br/>    protected_resource = object({<br/>      s3_bucket = object({<br/>        bucket_name     = string<br/>        object_prefixes = optional(list(string))<br/>      })<br/>    })<br/>  }))</pre> | `null` | no |
