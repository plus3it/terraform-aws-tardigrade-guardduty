...
|------|-------------|------|---------|:--------:|
| <a name="input_detector_features"></a> [detector\_features](#input\_detector\_features) | Provides a resource to manage a single Amazon GuardDuty detector feature. | <pre>list(object({<br/>    name   = string<br/>    status = string<br/>    region = optional(string)<br/>    additional_configuration = optional(object({<br/>      name   = string<br/>      status = string<br/>    }))<br/>  }))</pre> | `[]` | no |
